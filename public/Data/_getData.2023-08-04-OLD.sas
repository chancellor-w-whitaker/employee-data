...
%macro produceDatafile(inDate=); * inDate=23OCT2020 ;
	...
	Proc Summary data=primary nway missing;
		class START_DATE JOB_TYPE JOB_ECLS_GROUP JOB_ECLS_FT_PT PEAFACT_RANK_DESC TENURE_DESC GENDER computed_race;
		output out=summary (drop=_TYPE_ rename=(_FREQ_=total));
	Run;
	%resizeString(summary, newSize=100, List=JOB_ECLS_FT_PT GENDER computed_race);
...
%mend;
*%produceDatafile(inDate=23OCT2018);
...
Data metrics;
	...
	elementType = "X";
Run;
*%resizeString(metrics, newSize=200, List=division metric link description term);
